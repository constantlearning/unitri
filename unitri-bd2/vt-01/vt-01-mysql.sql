...
# drop table motorista_habilitacao CASCADE ;
...
select m.nome, v.marca, v.placa from motorista_veiculo
...
SELECT count(ch.data) as quantidade_chamados_mes
FROM boleto as b
INNER JOIN conveniada c on b.empresa_conveniada = c.id
INNER JOIN chamado_boleto cb on b.id = cb.id_boleto
INNER JOIN chamado ch on cb.id_chamado = ch.id
GROUP BY month(ch.data);


select month(data) from chamado;
select * from chamado_boleto;


-- 05) Listar nome dos cinco motoristas com maior faturamento mensal.
SELECT
  m.nome       as nome_motorista,
  sum(b.valor) as faturamento
FROM motorista AS m
  INNER JOIN motorista_boleto AS mb ON m.id = mb.id_motorista
  INNER JOIN boleto b on mb.id_boleto = b.id
  INNER JOIN chamado_boleto cb on b.id = cb.id_boleto
  INNER JOIN chamado c on cb.id_chamado = c.id
GROUP BY m.id, month(c.data)
HAVING c.data = 2
